...
public class ItemIngredientActual {
    @Actual
    public static MapCodec<Ingredient> NON_EMPTY_MAP_CODEC = makeCombinedIngredientMapCodec();
    ...
    private static MapCodec<Ingredient> makeCombinedIngredientMapCodec() {
    ...
    private static Ingredient convertToCompoundIngredient(Ingredient ingredient) {
        if (ingredient.getCustomIngredient() != null && ingredient.getItems().length != 1) {
            ...
            return DefaultCustomIngredients.any(Stream.of(ingredient.values).map(v -> Ingredient.fromValues(Stream.of(v))).toArray(Ingredient[]::new));
        }
        return ingredient;
    }
}
