...
public final class FluidItemLookup implements ItemLookup<CommonStorage<FluidUnit>, ItemContext>, RegistryEventListener {
    public static final FluidItemLookup INSTANCE = new FluidItemLookup();
    private static final ItemCapability<CommonStorage<FluidUnit>, ItemContext> CAPABILITY = ItemCapability.create(new ResourceLocation(BotariumTransfer.MOD_ID, "fluid_item"), CommonStorage.asClass(), ItemContext.class);

    private List<Consumer<ItemRegistrar<CommonStorage<FluidUnit>, ItemContext>>> registrars;
    ...
    public @Nullable CommonStorage<FluidUnit> find(ItemStack stack, ItemContext context) {
        CommonStorage<FluidUnit> unitContainer = stack.getCapability(CAPABILITY, context);
    ...
    @Override
    public void onRegister(Consumer<ItemRegistrar<CommonStorage<FluidUnit>, ItemContext>> registrar) {
    ...
    public void register(RegisterCapabilitiesEvent event) {
        registrars.forEach(registrar -> registrar.accept((getter, items) -> {
            ...
            event.registerItem(Capabilities.FluidHandler.ITEM, (stack, ignored) -> {
                ...
                CommonStorage<FluidUnit> container = getter.getContainer(stack, context);
